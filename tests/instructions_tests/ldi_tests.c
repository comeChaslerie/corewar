...
Test(load_ind_instr, loads_value_with_idx_modulo)
{
    main_t *main = create_test_main(1, "LdiBot");
    ...
    set_arena_value(main, 188, 0x10203040);
    set_arena_value(main, 700, 0x55667788);
    cr_assert(load_ind_instr(main, args, 0));
    cr_assert_eq(read_u32(main->robots[0].game_infos->regs[1]), 0x10203040);
    cr_assert(!main->robots[0].game_infos->carry);
    destroy_test_main(main);
}

Test(load_ind_instr, zero_loaded_value_sets_carry)
{
    main_t *main = create_test_main(1, "LdiBot");
    unsigned char value_a[REG_SIZE] = {0};
    unsigned char value_b[REG_SIZE] = {0};
