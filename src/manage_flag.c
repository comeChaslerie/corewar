...
static bool check_following_nb(int argc, char **argv, unsigned int *index, int *nbr)
{
    *index += 1;
    if (*index > argc)
        return put_error("Missing argument after a flag.", false);
    if (!is_nbr(argv[*index]))
        return put_error("The argument for the flag isn't in the right format.", false);
...
bool manage_flag_load(int argc, char **argv, unsigned int *index, robot_args_t *robots_args)
{
    if (my_strcmp(argv[*index], "-a") == 0) {
        if (robots_args->load_pos != -1)
            return put_error("Too many -a flags for one robot.", false);
        if (!check_following_nb(argc, argv, index, &robots_args->load_pos))
            return false;
        if (robots_args->load_pos > MEM_SIZE || robots_args->load_pos < 0)
            return put_error("The argument for the -a flag is out of range.", false);
        *index += 1;
    }
    return true;
}
...
bool manage_flag_id(int argc, char **argv, unsigned int *index, robot_args_t *robots_args)
{
    if (my_strcmp(argv[*index], "-n") == 0) {
        if (robots_args->id != -1)
            return put_error("Too many -n flags for one robot.", false);
        if (!check_following_nb(argc, argv, index, &robots_args->id))
            return false;
        if (robots_args->id > 4 || robots_args->id < 0)
            return put_error("The argument for the -n flag is out of range.", false);
        *index += 1;
    }
    return true;
}
